...
## Current State
- **Focus:** Wingman conversion landed end-to-end and visually verified by the user on 2026-05-02. Frame artwork stretched to a 1252×731 canvas with a 16:9 video rect at (136, 94) 980×551; passive Wingman background covers the cutout when nothing is loaded; round red 40×40 hammer+screwdriver settings button at (1133, 615) on the frame's bottom-right; F8 default hotkey; Discord URL pointed at ShipBit; splash redone with full-bleed Wingman art. `window.__wingmanLoad(videoId, playlistId)` is the prepared entry point for the future Wingman YouTube skill.
- **Blocked:** Wingman YouTube skill (TBD) not yet built — no host-side bridge that calls `__wingmanLoad`. Window title / assembly name still say "PulseNet Player" — full rebrand deferred. `Assets/icon.ico` is still the old PulseNet icon (works, but a placeholder).
